...
public class Pawn  extends Piece{
	...
	public String [] getAllMoves() {
		...
		if(this.getColor() == "white" && row == 6) {
						
			if(borderControl(row - 1, col)) {
				allMoves +=convert(row  - 1, col) + ",";
			}
			if(borderControl(row - 1, col - 1)) {
				allMoves +=convert(row  - 1, col - 1) + ",";
			}
			if(borderControl(row - 1, col + 1)) {
				allMoves +=convert(row  - 1, col + 1) + ",";
			}
			if(borderControl(row - 2, col)) {
				allMoves +=convert(row  - 2, col) + ",";
			}
		}	
